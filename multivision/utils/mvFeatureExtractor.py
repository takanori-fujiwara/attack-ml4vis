...
from .helper import charts_to_features
...
def compose_feature(chart, mv_charts):
    ...
    mv_charts = [
        c for c in mv_charts if chart['indices'] != c['indices']
        and chart['chart_type'] != c['chart_type']
    ]
...
def chart_to_feature(chart, mv_charts, all_charts_with_normed_score):
    ...
    score = [
        x for x in all_charts_with_normed_score
        if x['indices'] == chart['indices']
        and x['chart_type'] == chart['chart_type']
    ][0]['final_score']
    ...
    # print(chart)
    return [
        len(chart['indices']), *chart_type_feature(chart),
        compose_feature(chart, mv_charts),
        complementary_feature(chart, mv_charts), score
    ]


def charts_to_features_dl(mv_charts,
                          all_charts_with_normed_score,
                          seq_length=False):
    features = []
    for c in mv_charts:
        features.append(
            chart_to_feature(c, mv_charts, all_charts_with_normed_score))

    if seq_length != False:
        ## padding zero to match seq_length
        padding_zero = np.zeros(
            (seq_length - len(features), len(features[0]))).tolist()
...
def mvRecord_to_features(mvRecord, all_charts_with_normed_score):
    charts = [
        c for c in mvRecord['charts']
        if len(c['indices']) and not None in c['indices']
    ]
...
def get_all_charts_scores(charts):
    ...
    for idx, c in enumerate(charts):
        all_charts_with_normed_score.extend(
            zipChartsWithType(c, all_charts_scores_normed[idx]))

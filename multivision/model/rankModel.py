    ...
    def __init__(self, feature_size, linear_layers=[256, 128, 64, 8]):
        ...
        for i in range((len(linearLayers) - 1)):
            self.dense.add_module(
                "linear{0}".format(i),
                nn.Linear(linearLayers[i], linearLayers[i + 1]))
            self.dense.add_module("rulu{0}".format(i), nn.LeakyReLU())
            self.dense.add_module("dropout{0}".format(i),
                                  nn.Dropout(dropout_rate))

        self.dense.add_module("linear{0}".format(len(linearLayers)),
                              nn.Linear(linearLayers[-1], 1))
...
class ScoreNetLSTM(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(bidirectional=bidirectional,
                            ...
                            num_layers=1,
                            batch_first=True)
        self.linear = nn.Linear(hidden_size * seq_length * number_direction,
                                output_size)

####
# Enhanced version of Parametric UMAP.
# Supervised learning is added to the original Parametric UMAP
# Original source: https://github.com/lmcinnes/umap/blob/master/umap/parametric_umap.py
####
...
class ParametricUMAP(UMAP):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = (
            classifier  # neural network used for embedding to predict labels
        )
    ...
    def fit(self, X, y=None, precomputed_distances=None):
        if y is not None:
            ...
            self.n_classes = len(np.unique(y[y != None]))
            y = None  # to avoid using, ordinary umap's suvervised learning
    ...
    def _define_model(self):
        ...
        if self.parametric_embedding:
            ...
            if self.classification_loss_weight > 0:
                embedding_to_class = self.classifier(embedding_to)
                # stop gradient of reconstruction loss before it reaches the encoder
                # embedding_to_class = self.classifier(
                # embedding_to, tf.stop_gradient(embedding_to))
                embedding_to_class = tf.keras.layers.Lambda(
                    lambda x: x, name="classification"
                )(embedding_to_class)
                outputs["classification"] = embedding_to_class
        else:
    ...
    def _compile_model(self):
        ...
        if self.classification_loss_weight > 0:
            losses["classification"] = tf.keras.losses.CategoricalCrossentropy()
            loss_weights["classification"] = self.classification_loss_weight
    ...
    def _fit_embed_data(self, X, n_epochs, init, random_state):
        ...
        n_data = len(X)
        self.encoder, self.decoder, self.classifier = prepare_networks(
            ...
            parametric_embedding=self.parametric_embedding,
            parametric_reconstruction=self.parametric_reconstruction,
            classification=self.classification_loss_weight > 0,
        ...
        if classification:
            classifier = tf.keras.models.Sequential(
                [
                    tf.keras.layers.InputLayer(input_shape=n_components),
                    tf.keras.layers.Dense(
                        units=n_classes, name="classification", activation="softmax"
                    ),
                ]
            )
...
def construct_edge_dataset(
    ...
    def gather_X(edge_to, edge_from):
        ...
        return edge_to_batch, edge_from_batch, edge_to, edge_from

    def get_outputs(edge_to_batch, edge_from_batch, edge_to, edge_from):
        outputs = {"umap": tf.repeat(0, batch_size)}
        # outputs = {"umap": edge_to}
        ...
        if classification_loss_weight > 0:
            outputs["classification"] = tf.one_hot(tf.gather(y, edge_to), n_classes)
